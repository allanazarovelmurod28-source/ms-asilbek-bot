...
def get_conn():
    """Doimiy (persistent) ulanishni qaytaradi - har safar yangidan ulanmaydi.
    Agar ulanish uzilib qolgan bo'lsa (masalan uzoq vaqt harakatsizlikdan keyin),
    avtomatik qayta ulanadi."""
    global _conn
    if _conn is None or _conn.closed:
        _conn = psycopg2.connect(config.DATABASE_URL)
        _conn.autocommit = False
        log.info("Supabase bazasiga yangi ulanish ochildi")
    else:
        try:
            with _conn.cursor() as cur:
                cur.execute("SELECT 1")
        except psycopg2.Error:
            log.warning("Ulanish uzilgan edi, qayta ulanmoqda...")
            _conn = psycopg2.connect(config.DATABASE_URL)
            _conn.autocommit = False
    return _conn
